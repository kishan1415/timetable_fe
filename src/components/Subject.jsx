...
const onFinish = async (values) => {
  ...
  try {
    const response = await fetch("http://localhost:3001/subjects-entry", {
      method: "POST",
      headers: {
...
function Summary() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "createDate",
      render: (key) => <span>{dayjs(key).format("DD/MM/YYYY HH:mm")}</span>,
    },
    {
      title: "Action",
      key: "action",
        ...
        const handleDelete = async (e) => {
          ...
          try {
            ...
            const response = await fetch(
              `http://localhost:3001/subject/${record.id}`,
              {
                method: "DELETE",
                headers: {
                  Accept: "application/json",
  ...
  const getTimeTableData = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3001/subject-list");
      const dt = await response.json();
      setData(dt);
      setIsLoading(false);
    } catch (error) {
