...
const onFinish = async (values) => {
  ...
  try {
    const response = await fetch("http://localhost:3001/rooms-entry", {
      method: "POST",
      headers: {
...
function Summary() {
        ...
        const handleDelete = async (e) => {
          ...
          try {
            ...
            const response = await fetch(
              `http://localhost:3001/rooms/${record.id}`,
              {
                method: "DELETE",
                headers: {
                  Accept: "application/json",
  ...
  const getTimeTableData = async () => {
    ...
    try {
      const response = await fetch("http://localhost:3001/rooms-list");
      const dt = await response.json();
      setData(dt);
      setIsLoading(false);
    } catch (error) {
